...
/// Builder for the "get spending by category" action.
///
/// No additional configuration needed — just `.then` to transition.
class GetSpendingWhenBuilder extends DomainObjectBuilder<NeedsThen> {
  GetSpendingWhenBuilder(ScenarioBuilder<NeedsThen> scenario)
  ...
  void applyToContext(TestContext ctx) {
    ...
    ctx.set('_expenseAction', () async {
      try {
        ...
        final spending = await controller.getSpendingByCategory();
        return _SpendingResult.success(spending);
      } catch (e) {
        return _SpendingResult.failure(e.toString());
      }
    });
  }
  ...
  ExpenseThenBuilder get then {
    ...
    final withExecution = finalized.appendStep(
      StepRecord(
        ...
        action: (ctx) async {
          final action =
              ctx.get<Future<_SpendingResult> Function()>('_expenseAction');
          final result = await action();
          if (result.isSuccess) {
            ctx.set('spendingMap', result.spending!);
...
class _SpendingResult {
  _SpendingResult.success(this.spending)
      : isSuccess = true,
        error = null;

  _SpendingResult.failure(this.error)
      : isSuccess = false,
        spending = null;
